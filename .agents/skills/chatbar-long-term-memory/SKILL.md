...
- Persisted models: Android `data/local/entity/LongTermMemory.kt`, `AppSettings.kt`, and `SaveSlot.kt`; authoritative `ChatMessage.kt` and `ChatSession.kt` live in `app/sharedCore/src/main/kotlin/com/example/chatbar/data/local/entity/`.
- Storage: Android `data/repository/MemoryRepository.kt`; authoritative `ChatRepository.kt` lives in `app/sharedCore/src/main/kotlin/com/example/chatbar/data/repository/`. `JsonFileStorage.saveAll` is per-file, not a batch transaction; partial success updates only completed cache entries. `commitStateLast` persists a journal before nodes/revisions and advances the state pointer last; `getState` replays interrupted journals. `MemoryRepositoryDeletionJournalTest` covers partial-node-batch restart, replay idempotence, and cleanup before/after state publication.
...
- Persist manually interrupted assistant drafts with nonblank body or reasoning through `ChatRepository`, preserving the current user source turn. Reasoning-only regeneration selects a new empty-body version so old text cannot reenter chat history. Refresh the durable timeline before clearing same-ID streaming state and releasing the responding gate, then enqueue maintenance. Fully empty placeholders remain excluded; interrupted drafts skip full-completion-only side effects.
