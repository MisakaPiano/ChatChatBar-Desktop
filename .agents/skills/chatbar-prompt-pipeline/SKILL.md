...
- GENERAL_* replaces the former AI_TASK_* confirmation constants. Feature generation/repair systems remain the protocol owners. The earlier non-chat prompt cleanup still awaits user review. Main-chat SYSTEM_PROMPT_TEMPLATE, POST_HISTORY_INSTRUCTIONS_TEMPLATE and CCB constants were restored to the committed wording except two omitted minor-sexual passages in the system template and contract acknowledgement. Stored user/card prompts are not rewritten.
