...
- Import/export/card packages: shared contract and Character materialization authority under `app/sharedCore/src/main/kotlin/com/example/chatbar/domain/card/` (`CardTransferModels.kt`, `CharacterCardTransferCore.kt`, `CharacterResourceStore.kt`, `CharacterPlaceholderPolicy.kt`, `PngTextChunks.kt`, `FormatCardTransferService.kt`, `WorldBookTransferService.kt`); Android Character facade/adapters in CharacterCardTransferService.kt and AndroidCharacterTransferAdapters.kt, with CharacterCardPngRenderer.kt, SillyTavernCardParser.kt, and SillyTavernCardMapper.kt remaining Android-owned; SAF export contract in ui/components/CreateOpenableDocument.kt; export UI in ManageScreen.kt. Use chatbar-shared-import for external ACTION_SEND/ACTION_VIEW/text routing, content detection, FIFO staging, conflicts, unknown-type recovery, and shared-image handoff.
