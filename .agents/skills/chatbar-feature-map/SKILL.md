...
- Import/export/card packages: shared contract `app/sharedCore/src/main/kotlin/com/example/chatbar/domain/card/CardTransferModels.kt`, `CharacterPlaceholderPolicy.kt`, and `PngTextChunks.kt`; Android orchestration CharacterCardTransferService.kt, CharacterCardPngRenderer.kt, SillyTavernCardParser.kt, and SillyTavernCardMapper.kt; SAF export contract in ui/components/CreateOpenableDocument.kt; export UI in ManageScreen.kt. Use chatbar-shared-import for external ACTION_SEND/ACTION_VIEW/text routing, content detection, FIFO staging, conflicts, unknown-type recovery, and shared-image handoff.
