...
- `app/sharedCore/src/main/kotlin/com/example/chatbar/domain/prompt/CharacterNaiPromptDefaults.kt` for the Character default-negative Prompt authority; Android callers keep using the PromptTemplates facade.
...
- `NOVELAI_IMAGE_PROMPT_SYSTEM` is the V4.5 Full system prompt; its old override/development-test preamble was cleaned without changing visual rules or JSON protocol. The reference-image helpers received the same cleanup. `NOVELAI_IMAGE_PROMPT_SYSTEM_V5` derives the model name, 1000/150-token budgets, relaxed brevity, Chinese/English prose + Danbooru mixing, and prose as a last resort for niche needs without tags. Studio revision helpers explicitly preserve unrequested Chinese, prose, and weights without deletion or translation. Route shared systems through `novelAiImagePromptSystem(...)` or `novelAiImagePromptCoreSystem(...)` with an explicit `NovelAiImageModel`.
...
- Send final user-specific image requirements through `novelAiImagePromptPreferenceUser(...)` as the feature's last user-role message before GENERAL_* wrapping. They remain the final actual input message after planner/revision assistant history; the envelope itself ends with GENERAL_POST_USER_IDENTITY_REMINDER_USER_PROMPT. Do not move these requirements into a system message.
...
- When `ImageUnderstandingService.prepare` supplies descriptions for a non-multimodal design model, pass them as research `existingSceneDescription`: reuse `decideQueriesOnly` for queries and retain the original description for codex recall and final Prompt design, including query-planning failure. Never re-generate that scene. Direct multimodal planning keeps its existing scene-plus-queries path.
...
- Bundled style previews use 512×512 WebP at quality 80 for the small gallery; `previewImage` in the style catalog owns each filename.
- Default negative tags live in shared Prompt authority `CharacterNaiPromptDefaults.DEFAULT_CHARACTER_NAI_NEGATIVE_PROMPT`; `PromptTemplates.DEFAULT_CHARACTER_NAI_NEGATIVE_PROMPT` remains the Android-compatible facade. Card-level `CharacterCard.defaultImageNegativePrompt` flows into `NovelAiPromptPlan.negativePrompt`, with the template used only when that value is blank. `NovelAiImageService` disables quality-tag injection, uses `ucPreset=3` (None), and sends that one effective negative text identically through legacy and V4 fields without combining another preset.
...
- `NOVELAI_IMAGE_NATURAL_LANGUAGE_PROMPT_SYSTEM_V5` is the dedicated feature-mode exception shared by studio AI design and opt-in chat image design: Chinese base/role prose preserves English character tags, `source#`/`target#`/`mutual#` interaction syntax, NAI weights, structured JSON, and V5 1000/150-token budgets. Keep it V5-only and route through `novelAiImageNaturalLanguagePromptCoreSystem`.
- Chat persists `ChatSession.novelAiNaturalLanguageMode` (default false). ChatViewModel snapshots the setting with the resolved image model; only V5 enables `design(naturalLanguageMode = true)`. Reuse studio's natural-language research evidence and JSON repair, skip Tag post-processing, and retain chat anchor/player markers, character references, locked style, negative prompt, and six-character cap. V4.5 suspends the saved preference; Moments and existing-Prompt regeneration do not use it. Routing/legacy JSON fixtures: `NovelAiChatNaturalLanguageModeTest`.
...
- Studio AI design is a persisted conversation. Natural-language mode forces the turn target to V5 Full, then keeps the full first-round scene planning, local Danbooru catalog lookup, codex, final AI design, JSON repair, and initial evidence snapshot. Its dedicated system emits Chinese natural-language `baseCaption` plus ordered character captions with preserved English identity/appearance tags, interaction syntax, and weights; skip tag canonicalization/post-processing so Chinese prose remains intact. Default structured mode uses the shared model-version system and normal post-processing. Later rounds in either mode use the previous valid structured reply as authority unless that turn explicitly attaches a Studio Prompt snapshot; attached snapshots become the revision authority and reset older initial evidence for that lineage. Revision rounds use `researchForRevision`: recall codex from only the raw user modification request (no prior Prompt, character reference, extra requirement, or planned queries), then plan optional new Danbooru queries. Codex recall survives empty/failed Tag planning. Inject applicable baseline plus current-round Tag/codex evidence, deduplicate codex IDs, and keep the modification request as the final user message. Do not accumulate intermediate-round evidence.
- Each studio AI-design conversation owns an immutable snapshot of imported character references and extra requirement, but deliberately excludes implicit current editable base/character Prompt because it may be an earlier conversation's applied reply. The explicit one-shot “attach current Studio Prompt” action is the only exception: persist that turn's base/ordered-character positive snapshot without style, negatives, or settings; use it instead of every earlier reply as the revision baseline, and keep retries/regeneration on the saved snapshot. That attachment resets old initial Tag/codex evidence for its revision lineage while retaining the conversation's imported character references and extra requirement. Legacy conversations without a context snapshot use an empty snapshot and must never borrow the current studio draft.
- Retrying a failed studio turn preserves that turn's saved natural-language mode and target model. Deliberate regeneration uses the currently selected natural-language mode; enabling it forces the regenerated turn to V5 Full, while disabling it returns to structured Tag output without changing the turn's saved target model.
- Studio user bubbles expose editing and regeneration for any historical turn. `editTurnAndCreateCurrentConversation` creates a new current conversation containing the unchanged prefix and edited pending turn; the original conversation remains in history. Preserve context, saved attachment and image mode, discard the edited reply and descendants only from the new branch, and clear initial research when editing the first turn. `editAndRetryTurn` uses the selected design model; cancellation/failure leaves the edited turn retryable. No schema migration is needed.
- NovelAI scene/query planning, final Prompt design, revision, and JSON repair inherit the selected text model's thinking configuration without task budget or effort overrides.
- Planner output contains only `sceneDescription` and `queries`; do not add `action`, `purpose`, or `reason`. The local catalog receives only planned queries and normalizes unavoidable English spaces to Danbooru underscores before lookup.
- Local codex compiler preserves each selected source file's complete `###` section as one reference block. Retrieval uses only Chinese text extracted from the whole section, scored by corpus-weighted Chinese two/three-character overlap against both detailed scene and queries; English prompt tags and rewrite aliases never affect local recall. Select the five most relevant blocks, randomize only their delivery order, and never penalize recently used blocks. Send each selected original section intact as optional final-design evidence. All local catalog lookups run concurrently.
- Prompt-tool reference images enter both planning and final design requests. Planning failure uses the original task text for local codex recall, exposes the failure in progress, skips the local catalog, and continues final design; cancellation still stops before final design.
...
- `DanbooruTagCatalog` is application-scoped and shared by research, studio live completion, and Prompt Chinese annotations. It installs the bundled compressed SQLite baseline lazily, keeps a version-keyed 128-entry memory cache, and has no credentials or user settings.
- Studio/shared/fullscreen live completion uses `NovelAiTagSuggestionService`: both sources stream individual matches into a ranked set, rendered at frame cadence while loading. It has no debounce or result cap; all five Danbooru categories retain descending `post_count`/name order, followed by alphabetic dictionary results with catalog-first deduplication. AI research alone retains its eight-candidate and general/copyright/character contract.
- `RankedTagIndex` uses versioned, pre-ranked UTF-16 single/bigram postings with delta-varint rank lists; lookup chooses the rarest gram, verifies full contains semantics, and emits each match immediately. Both offline and runtime builders omit the unused `entries_name` secondary index; reads use the rank primary key. Format remains 2, and existing files containing the optional index remain reusable. Bundle compiler: `tools/build_tag_completion_indexes.py`; package `.sqlite.binz` gzip payloads and verify with `tools/verify_tag_completion_assets.py <APK>`. Never use `.gz`, which AAPT expands and renames. Preparation runs in a shared background task outside the catalog mutex; cancellation detaches query waiters without cancelling the build. Hold source database references through preparation and page runtime posting construction by key. Row-returning PRAGMAs use rawQuery. Downloaded catalogs prepare their index before atomic activation; caches and editor guards remain source-version keyed.
- V5 quote expansion belongs to `NovelAiV5TextPromptPolicy`, shared by requests and token counting. It follows official web build 3102745/module 46278: ASCII/curly double and single quotes plus Japanese corner quotes, apostrophe boundaries, >30% CJK reversal within each field, first base chunk only, and a trailing `, teXt:` block. Character order stays unchanged with coordinates disabled. Explicit case-insensitive `text:` uses official boundaries and excludes `text::`/spaced colons. Keep stored/designed text unchanged; V4.5 bypasses expansion.
...
3. The scene plus queries recall local codex templates while queries concurrently retrieve local Danbooru catalog results.
