...
- Text-model scene/query planning declares IMAGE_RESEARCH/PLAN; design declares IMAGE_DESIGN with GENERATE or REPAIR. Shared transport inserts the neutral task confirmation without moving original evidence or final user preferences. Actual request logging is transport-owned; do not restore NovelAiPromptDesigner's old duplicate logical-body record. Source-only revision wording lives in PromptTemplates.novelAiRevisionWithCharacterReference.

- `NOVELAI_IMAGE_PROMPT_SYSTEM` is the unchanged V4.5 Full system prompt. `NOVELAI_IMAGE_PROMPT_SYSTEM_V5` derives the model name, 1000/150-token budgets, relaxed brevity, Chinese/English prose + Danbooru mixing, and prose as a last resort for niche needs without tags. Studio revision helpers explicitly preserve unrequested Chinese, prose, and weights without deletion or translation. Route shared systems through `novelAiImagePromptSystem(...)` or `novelAiImagePromptCoreSystem(...)` with an explicit `NovelAiImageModel`.
