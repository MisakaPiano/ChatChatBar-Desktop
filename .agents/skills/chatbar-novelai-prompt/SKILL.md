...
- Text-model scene/query planning declares IMAGE_RESEARCH/PLAN; design declares IMAGE_DESIGN with GENERATE or REPAIR. Shared transport applies the eight-message GENERAL_* envelope: feature/evidence systems are combined into the first system in order; non-system history and final preferences retain their relative order inside the fifth user input, followed by the fixed acknowledgement/reminder tail. Actual request logging is transport-owned; do not restore NovelAiPromptDesigner's old duplicate logical-body record. Source-only revision wording lives in PromptTemplates.novelAiRevisionWithCharacterReference.
