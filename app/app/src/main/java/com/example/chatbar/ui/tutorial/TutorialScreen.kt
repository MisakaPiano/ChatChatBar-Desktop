    ...
    TutorialPage(
        ...
        sections = listOf(
            ...
            TutorialSection(
                title = "分段回复",
                steps = listOf(
                    "开启“助手回复分段气泡”后，长按某个片段打开“片段操作”；菜单内容可上下滚动，键盘打开或屏幕空间不足时也能找到底部选项。",
                    "状态栏点击可展开或折叠；长按标题或正文打开片段操作，关闭分段气泡时打开整条消息操作。复制状态栏请使用操作菜单中的复制。",
                    "片段菜单左侧窄列操作“本段”，右侧宽列操作“整条消息”。两列从上到下的图标依次为复制、编辑、加入长截图、删除；右列底部的循环箭头重新生成整条回复，扳手修复整条消息格式。多选时点消息左上角小圆可勾选或取消整条，状态栏也可直接点选。",
                    "长按有明确说话人物且已绑定音色的对白或内心气泡，可选择“本段语音”或“整条语音”；听书模式下也可长按旁白块生成语音。",
        ...
        sections = listOf(
            TutorialSection(
                ...
                steps = listOf(
                    ...
                    "“参数与设定”按对话、角色与世界、玩家与背景、图片、语音、记忆六类整理；可搜索设置名称直接定位。",
