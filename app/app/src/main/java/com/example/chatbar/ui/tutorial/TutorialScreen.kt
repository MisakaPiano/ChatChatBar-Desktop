        ...
        sections = listOf(
            TutorialSection(
                ...
                steps = listOf(
                    ...
                    "长按生成图片，可分别修改画风 Prompt、基础 Prompt、角色提示词、负面提示词和图片比例。普通与全屏输入均支持中英文 Tag 预测和实时翻译，优先匹配 Danbooru，词典补充未命中项；可切换实时翻译，全屏确认保存、取消放弃。重新生成使用新 seed；不修改比例时保留原图尺寸，也可删除图片。",
                    "文本正在生成时也可长按图片并重新生图。",
                    "普通聊天图片长按会直接进入删除确认。"
                )
            ),
