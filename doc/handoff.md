...
> 状态更新：2026-09-15（下方早期功能记录不代表最新运行验证）
> 当前切片：共享 Tag 翻译层缓存中文排版，滚动仅偏移并裁掉离屏绘制；预测栏按查询重置位置，未主动滑动时保持排序起点，避免先到的词典项被后到的热门 Tag 推到末尾。实现与约束见 `.agents/skills/chatbar-image-generation-runtime/SKILL.md`。
> 验证：Release 构建与 lintVital 通过；无连接设备，未安装、未实测帧率、未运行自动测试。下一步手工检查长 Prompt 翻译滚动、短查询预测补齐、主动横滑保位和全屏/重新生图编辑器回归。
> 本轮只提交本地代码，不推送、不发布新版本；此前记录的已发布基线为 `v1.3.41`，本轮未查询远端。
...
设置整理（2026-09-13）：全局八分类、会话参数六分类与静态元信息搜索已实现；会话长期记忆/RAG/存档标签保留。`GlobalSettingsScreen.kt`、`SessionSettingsContent.kt` 使用 `SettingsBrowser.kt`；分类滚动独立，密钥/长文本/世界书/调试按需进入。普通草稿显式保存，离开与载入存档先处理草稿；主题/字号及密钥保留原生效时机。仓库只合并修改字段，保留后台状态；安全输入不保存页面恢复数据。会话全屏编辑在原 Dialog 的 Box 内覆盖显示，保留底层工具草稿。Debug Kotlin 与单元测试源码编译通过，自动测试未执行；最终 release 构建及 lintVital 通过，已保留数据安装并启动 PJZ110（49075ec2）。待手测分类搜索、草稿保存/放弃、即时外观与草稿共存、记忆后台更新、密钥、世界书、存档及小屏/大字体；未运行截图、日志或 UI 自动诊断。

当前切片已实现：聊天图片长按编辑弹窗及全屏编辑同步工作室画风/基础 Prompt 分栏、全量中英 Tag 预测和实时翻译。入口 `NovelAiImageRegenerationDialog.kt`；`NovelAiTagEditor.kt` 管理防抖、选择区和隔离全屏草稿，`NovelAiPromptRendering.kt` 共用工作室注释/换行绘制。全局翻译开关共享；Danbooru 优先，词典补充。新元数据记录画风前缀，旧图片保留完整基础原文、不猜画风；重新生成只拼一次。待手工验证长输入末尾翻译、中英文预测排序、source#/target#、全屏确认/取消、画风修改后重生图和旧图片兼容。
