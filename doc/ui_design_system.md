...
| `SettingsBrowser.kt` | 设置分类、元信息搜索、详情入口、48dp 设置开关、未保存提示及字段草稿同步 |
...
- 设置行和开关的触控目标至少 48dp；视觉尺寸与触控尺寸分离。
...
- 管理页：Tabs 分角色、格式、世界书、模型、设置；全局设置使用八分类首页。详情内禁用外层横向切页，普通设置统一由顶部保存。
- 会话设置：“参数与设定”使用六分类首页，保留长期记忆、RAG 检索库、存档标签。分类切换保留草稿及滚动位置；离开或载入存档前处理未保存修改。
- 设置搜索只索引静态标题和别名；世界书列表、密钥、长文本和调试各有独立编辑入口。主题、字号立即生效，密钥独立保存；未提交密钥不进入页面恢复状态。
